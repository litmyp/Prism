        ...
        if self.distill_output_type == 'discrete' or self.config.agent.ppo.discrete_actions:
            ...
            def to_idx(v):
                ...
                if float(v).is_integer():
                    return int(np.clip(int(v), 0, len(weights) - 1))
                # 否则按最接近的离散动作值匹配
                return int(np.argmin(np.abs(weights - float(v))))

            idxs = [to_idx(p) for p in preds]
            actions = torch.tensor(idxs, dtype=torch.long)
        else:
            actions = torch.tensor(preds, dtype=torch.float32)
        return actions
