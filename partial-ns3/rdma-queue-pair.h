...
#include "rl-interface.h" // lty added: 引入接口定义
#include <vector>
...
public:
	...
	// ==========================================
	// lty added: RL Statistics & Management
	// ==========================================
	int32_t m_rlSlotIndex; // 对应的共享内存槽位ID

	// 统计计数器 (由 RdmaHw 在运行时更新)
	uint32_t m_periodCnp;       // 周期内收到的CNP数
    uint32_t m_periodNack;      // 周期内收到的NACK数
	...
	uint32_t m_rlAccumMiCount;  // 连续合并的 monitor_interval 数（用于跳过 pkts=0 的 MI）
