...
int RdmaHw::ReceiverCheckSeq(uint32_t seq, Ptr<RdmaRxQueuePair> q, uint32_t size){
	...
	if (seq == expected){
		...
		if (Simulator::Now() >= q->m_nackTimer || q->m_lastNACK != expected){
            ...
            Ptr<RdmaQueuePair> qp = GetQp(q->sip, q->dport, q->m_ecn_source.qIndex);
            if (qp != NULL) {
                qp->m_periodNack++;
            }
			return 2;
...
void RdmaHw::RlSampleOnce(Ptr<RdmaQueuePair> qp){
    ...
    // 如果该 MI 内没有有效数据（无 pkts 或无 rtt），累加计数后直接进入下一个 MI
    if (qp->m_periodPkts == 0 || qp->m_periodRttCount == 0) {
        // 统计量保持累加，直到出现非零 pkts 且有 RTT 样本
        qp->m_rlAccumMiCount++;
    ...
    uint64_t rtt_mean_steps = qp->m_periodRttCount ? static_cast<uint64_t>(qp->m_periodRttSum / qp->m_periodRttCount) : 0;
    uint64_t rtt_mean_ns = TimeStep(rtt_mean_steps).GetNanoSeconds(); // 保持原始 ns 单位
    sample.rtt_ns = clamp32(rtt_mean_ns);
    sample.nack_count = qp->m_periodNack;
    sample.cnp_count = qp->m_periodCnp;
    sample.bytes_sent = clamp32(qp->m_periodBytes);
    // 速率归一化到 [0,1]，再放大 2^20 写入定点
    double norm = 0.0;
    if (qp->m_max_rate.GetBitRate() > 0) {
        norm = static_cast<double>(qp->m_rate.GetBitRate()) / static_cast<double>(qp->m_max_rate.GetBitRate());
    }
    norm = clamp_double(norm, 0.0, 1.0);
    uint64_t rate_fp20 = static_cast<uint64_t>(norm * static_cast<double>(1u << 20));
    sample.rate_fp20 = clamp32(rate_fp20);
    uint64_t interval_ns_total = static_cast<uint64_t>(m_moniterIntervalNs) * static_cast<uint64_t>(qp->m_rlAccumMiCount);
    sample.interval_ns = clamp32(interval_ns_total);
    sample.packets_sent = qp->m_periodPkts;
    sample.flow_id = qp->GetHash();
    sample.host_id = m_node ? static_cast<uint32_t>(m_node->GetId()) : 0;

    bool published = RLInterface::Get()->PublishSample(qp->m_rlSlotIndex, sample);

    if (published) {
        // 将 RL Sample 以 CSV 形式写入 temp.csv，避免与其它 stdout 混杂。
        static bool s_rlCsvInited = false;
        static std::ofstream s_rlCsvFile;
        if (!s_rlCsvInited) {
            s_rlCsvFile.open("temp.csv", std::ios::out | std::ios::trunc);
            s_rlCsvInited = true;
            if (s_rlCsvFile.is_open()) {
                s_rlCsvFile << "sim_time_ns,host,flow,slot,mi_ns,cnp,pkts,bytes,rtt_ns,rate_raw" << std::endl;
            } else {
                NS_LOG_WARN("RLInterface: failed to open temp.csv for RL samples");
            }
        }
        std::ostream& rl_out = s_rlCsvFile.is_open() ? s_rlCsvFile : std::cout;
        uint64_t sim_time_ns = Simulator::Now().GetTimeStep();
        rl_out << sim_time_ns << ','
               << sample.host_id << ','
               << sample.flow_id << ','
               << qp->m_rlSlotIndex << ','
        ...
        NS_LOG_WARN("RLInterface: failed to publish sample for slot " << qp->m_rlSlotIndex);
        qp->ResetRlStats();
        ScheduleRlSample(qp, false);
        return;
    }
    ...
    // lty added: 写入后阻塞等待外部动作，仿真暂停直到 Python 回写
    RLInterface::Get()->WaitForResult(qp->m_rlSlotIndex);

    // lty added: 读取外部写回的结果值作为速率更新系数
    double factor = 1.0;
    if (RLInterface::Get()->GetResultValue(qp->m_rlSlotIndex, factor)) {
        const double kMinFactor = 0.8;
        const double kMaxFactor = 1.2;
        double original = factor;
        factor = clamp_double(factor, kMinFactor, kMaxFactor);
        if (original != factor) {
            NS_LOG_WARN("RLInterface: action clamped from " << original << " to " << factor
                         << " for slot " << qp->m_rlSlotIndex);
        }
        uint64_t cur_bps = qp->m_rate.GetBitRate();
        uint64_t new_bps = static_cast<uint64_t>(cur_bps * factor);
        ...
        ChangeRate(qp, DataRate(new_bps));
        std::cout << "收到来自python的更新：factor=" << original
                  << " (clamped " << factor << ") new_bps=" << new_bps << std::endl;
    } else {
        NS_LOG_WARN("RLInterface: no action received for slot " << qp->m_rlSlotIndex);
    }
